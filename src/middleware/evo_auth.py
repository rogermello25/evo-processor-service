...
class EvoAuthMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        if hasattr(request, 'url') and self._should_skip(request.url.path):
            return await call_next(request)
        ...
        if '/sync/' in request.url.path:
            ...
            if x_api_key:
                ...
                agent_id = self._extract_agent_id_from_sync_path(request.url.path) if hasattr(request, 'url') else None
                if agent_id:
                    ...
                    logger.warning(f"EvoAuth: Could not extract agent_id from /sync/ path: {(request.url.path if hasattr(request, 'url') else 'N/A')}")
        ...
        try:
            ...
            if not auth_response:
                ...
                agent_id = self._extract_agent_id_from_path(request.url.path) if hasattr(request, 'url') else None
